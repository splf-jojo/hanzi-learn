...
def _seed(db: Session, markdown_dir: Path, only_if_empty: bool) -> ParsedData | None:
    if only_if_empty and db.scalar(select(Word.id).limit(1)) is not None:
        return None
...
def seed_database(
    ...
    if db is not None:
        return _seed(db, markdown_dir, only_if_empty)

    with SessionLocal() as session:
        return _seed(session, markdown_dir, only_if_empty)
